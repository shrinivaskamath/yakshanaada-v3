...
function TanpuraM() {
  ...
  React.useEffect(() => {
    return () => {
      // Clean up sounds
      analytics().logEvent('tanpura_m_start');
  ...
  const playSoundInLoop = (file: string) => {
    analytics().logEvent('tanpura_m_playing_' + file);
    analytics().logEvent('tanpura_m_fade_eable_' + fadeEnabled);
